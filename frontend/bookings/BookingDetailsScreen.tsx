...
import {View, Text, StyleSheet, Image, TouchableOpacity, ImageBackground, SafeAreaView, Linking, Platform} from "react-native";

const calculateTotalPrice = (pickupDate: string, dropoffDate: string, pricePerDay: number) => {
    const start = new Date(pickupDate);
    const end = new Date(dropoffDate);
    const days = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24));
    return (days * pricePerDay).toString();
};

// SOME BACKEND IS NEEDED TO HANDLE CANCELLATIONS OF BOOKINGS
export default function BookingDetailsScreen({ route, navigation }: any) {
    ...
    // Function to open address in maps, not tested for android yet only ios emulator
            ...
            <SafeAreaView style={styles.safeArea}>
                <View style={styles.overlay}>
                    ...
                    <View style={styles.info}>
                        <Text style={styles.car}>{booking.car}</Text>
                        <Text style={styles.bookingNumber}>
                            Booking #{booking.bookingNumber}
                        </Text>
                        <Text style={styles.price}>Total: {totalPrice} DKK</Text>
                        ...
                        <Text style={styles.value}>{formatDate(booking.dropoffDate)}</Text>
